...
class HydroElectric_Test(gym.Env):
    ...
    def __init__(self, path_to_test_data:str):
        ...
        try:
            ...
            # Fallback to CSV if Excel fails (likely for your .csv files)
            self.test_data = pd.read_csv(path_to_test_data)

        # Ensure the column name for prices is correct
        # Your CSV likely has a specific header. If 'PRICES' fails, check the file header.
        # Based on your upload, you might need to rename the column dynamically or ensure consistency.
        if 'PRICES' not in self.test_data.columns:
            # Attempt to find the price column if named differently (e.g. 'Price', 'value')
            # Or simply rename the first valid column:
            self.test_data.rename(columns={self.test_data.columns[0]: 'PRICES'}, inplace=True)
